...
const Register4 = (props) => {
  ...
  const [modal3, setModal3] = useState(false); //내 공장1 정보 등록
	...
	const [toastModal2, setToastModal2] = useState(false);
  ...
  const [mbHp, setMbHp] = useState(route.params.mbHp);
	...
	const [pw, setPw] = useState(route.params.mbPw);
  ...
  function findMyPosition(v){
	...
	function chkFactoryInfo(v){
		if(v == "1"){
			...
			if(factName2 == ''){
				setToastText('공장명을 입력해 주세요.');
				setToastModal2(true);
				setTimeout(()=>{ setToastModal2(false) },1000);
				return false;
			}
	...
	function submitRegist(){
		...
		Api.send('POST', 'member_save', formData, (args)=>{
			let resultItem = args.resultItem;
			let responseJson = args.responseJson;
