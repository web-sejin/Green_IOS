...
const UsedChat = ({navigation, route}) => {
  ...
  const moreData = async () => {    
    if(totalPage > nowPage){
      await Api.send('GET', 'list_chat_product_user', {is_api: 1, pd_idx: idx, page:nowPage+1}, (args)=>{
        let resultItem = args.resultItem;
        let responseJson = args.responseJson;
        let arrItems = args.arrItems;
  ...
  const getList = ({item, index}) => (    
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        navigation.navigate('ChatRoom', {pd_idx:idx, page_code:'product', recv_idx:item.mb_idx, roomName:roomName, cr_idx:item.cr_idx});
      }}
    >
